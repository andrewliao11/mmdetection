...
@HEADS.register_module()
class ExtractFeatDETRHead(DETRHead):
    ...
    def __init__(self,
        ...
        self.sync_cls_avg_factor = sync_cls_avg_factor
        class_weight = loss_cls.get('class_weight', None)
        if class_weight is not None and (self.__class__ is ExtractFeatDETRHead):
            assert isinstance(class_weight, float), 'Expected ' \
                'class_weight to have type float. Found ' \
                f'{type(class_weight)}.'
            # NOTE following the official DETR rep0, bg_cls_weight means
    ...
    def forward_single(self, x, img_metas):
        ...
        pos_embed = self.positional_encoding(masks)  # [bs, embed_dim, h, w]
        # outs_dec: [nb_dec, bs, num_query, embed_dim]
        outs_dec, encoder_outs = self.transformer(x, masks, self.query_embedding.weight,
                                       pos_embed)
    ...
    def simple_test_bboxes(self, feats, img_metas, rescale=False):
        ...
        # forward of this head requires img_metas
        outs = self.forward(feats, img_metas)
        all_cls_scores, all_bbox_preds, encoder_outs = outs
        results_list = self.get_bboxes(*[all_cls_scores, all_bbox_preds], img_metas, rescale=rescale)
        
        new_results_list = []
        for a, b in zip(results_list, encoder_outs):
            a = list(a)
            a.append(b)
